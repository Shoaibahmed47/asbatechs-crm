      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          <div className="rounded-2xl border border-emerald-200/80 bg-emerald-50/40 px-4 py-2 dark:border-emerald-900/40 dark:bg-emerald-950/30">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-emerald-800 dark:text-emerald-400">Active</div>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {people.map((p) => (
              <tr key={p.userId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                ...
                >
                  {p.breakIsOpen ? <span className="text-amber-700 dark:text-amber-400">- (open)</span> : formatTime(p.breakEnd)}
                </td>
              </tr>
            ))}
          </tbody>
