...
function StatusPill({ status }: { status: PersonAttendanceStatus["status"] }) {
  ...
  return (
    <span
      className={`inline-flex rounded-full px-2.5 py-0.5 text-[11px] font-semibold uppercase tracking-wide ${styles}`}
    >
      {label}
    </span>
  );
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="rounded-2xl border border-amber-200/80 bg-amber-50/40 px-4 py-2 dark:border-amber-900/40 dark:bg-amber-950/30">
            <div className="text-[10px] font-semibold uppercase tracking-wider text-amber-900 dark:text-amber-400">
              On break
            </div>
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
            {people.map((p) => (
              <tr key={p.userId} className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40">
                ...
                <td className="px-4 py-2.5 text-slate-600 dark:text-slate-400">{p.email}</td>
                ...
                >
                  {formatTime(p.breakStart)}
                </td>
                <td
                  className="px-4 py-2.5 text-slate-700 dark:text-slate-300"
                  title={
                    p.breakIsOpen
                      ? "Break still open — end time after employee ends break"
                      : p.breakEnd
