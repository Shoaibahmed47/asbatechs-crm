...
describe("attendance agent status route", () => {
  const req = () =>
    ({
  ...
  it("returns not installed when no agent activity exists", async () => {
    ...
    whereMock
      .mockImplementationOnce(() => Promise.resolve([])) // today log
      .mockImplementation(() => ({ orderBy: orderByMock })); // latest signal query chains
