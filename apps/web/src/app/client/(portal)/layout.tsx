...
export default async function ClientPortalLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="client-portal-shell min-h-screen bg-slate-100/80 text-base leading-relaxed text-slate-900 dark:bg-slate-950 dark:text-slate-100">
          ...
          <nav className="flex flex-wrap items-center gap-1">
            {nav.map((item) => (
              <Link
                key={item.href}
                href={item.href}
                className="rounded-lg px-3.5 py-2.5 text-base text-slate-600 transition hover:bg-slate-100 hover:text-slate-900 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
              >
                {item.label}
              </Link>
            ))}
